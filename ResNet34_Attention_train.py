...
from model.ResNet_Attention import resnet_34
...
test_transform = transforms.Compose([
    transforms.RandomAffine(degrees=0, scale=(1, 2), shear=45),
    transforms.ToTensor(),
    # transforms.Normalize(mean=[0.485, 0.456, 0.406],
    #                      std=[0.229, 0.224, 0.225])
    transforms.Normalize(0.5, 0.5)
...
train_dataset = datasets.MNIST(root='MNIST', train=True, download=True, transform=train_transform)
train_loader = DataLoader(train_dataset, shuffle=True, batch_size=batch_size)

test_dataset = datasets.MNIST(root='MNIST', train=False, download=True, transform=test_transform)
test_loader = DataLoader(test_dataset, shuffle=False, batch_size=batch_size)
model = resnet_34(num_classes=10, in_channel=1)
...
if __name__ == '__main__':
    ...
    filename = 'resnet34_attention.txt'
